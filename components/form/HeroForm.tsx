...
const HeroForm = () => {
  ...
  const inputClass = 
    "w-full bg-[#111] border border-[#333] text-gray-300 rounded-lg px-4 py-4 appearance-none focus:outline-none focus:border-[#BF953F]/50 focus:text-white transition-colors text-sm font-light";
  ...
  return (
    ...
    >
      {/* Container Box: Dark with Thin Gold Border */}
      <div className="bg-black/80 backdrop-blur-md border border-[#BF953F]/40 rounded-3xl p-8 md:p-12 shadow-2xl">
        
        {/* Heading */}
        <h3 className="text-center text-white tracking-[0.15em] uppercase font-semibold text-sm md:text-base mb-8">
          Enter your details to get started
        </h3>

        <form onSubmit={handleSubmit} className="flex flex-col items-center gap-8">
          
          {/* Inputs Grid: 3 Columns on Desktop, 1 Column on Mobile */}
          <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
            ...
            <div className={inputWrapperClass}>
              ...
              >
                <option value="" disabled>What do you drive?</option>
                {HERO_DATA.carMakes.map((make) => (
                  <option key={make.value} value={make.value} className="bg-[#111]">
                    {make.label}
                  </option>
                ))}
              </select>
            ...
            <div className={inputWrapperClass}>
              ...
              >
                <option value="" disabled>Which model?</option>
                {HERO_DATA.carModels.map((model) => (
                  <option key={model.value} value={model.value} className="bg-[#111]">
                    {model.label}
                  </option>
                ))}
              </select>
            ...
            <div className={inputWrapperClass}>
              ...
              <button
                ...
                onClick={handleLocationClick}
                disabled={isLocating}
                className="absolute right-2 top-1/2 -translate-y-1/2 p-2 text-gray-500 hover:text-[#BF953F] transition-colors"
              >
                {isLocating ? (
                  <div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" />
                ) : (
          ...
          <button
            type="submit"
            className="px-12 py-3 bg-gradient-to-b from-[#D4AF37] to-[#8B7328] hover:from-[#E5C158] hover:to-[#9E8430] text-white text-sm font-medium tracking-wide uppercase rounded-lg shadow-lg transition-all transform hover:scale-105"
          >
            Get Quotes
          </button>
        </form>
